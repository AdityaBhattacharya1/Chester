...
An identifier may be declared at most once as a global, and at most once as a local in any particular function; however, an identifier may appear as a local in many different functions.
A function may have at most one prototype; a function may be defined at most once.
If a function has a prototype, then the types of the formals at its definition must match (i.e., be the same), in number and order, the types of the argument in its prototype; and the type of the return value at its definition must match the type of the return value at its prototype.
The prototype, if present, must precede the definition of the function.

An identifier can occur at most once in the list of formal parameters in a function definition.
The formal parameters of a function have scope local to that function.
If a function takes no parameters, its prototype must indicate this by using the keyword void in place of the formal parameters.
...
int is compatible with int, and char is compatible with char;
int is compatible with char, and vice versa;
an array of int is compatible with an array of int, and an array of char is compatible with an array of char; and
any pair of types not covered by one of the rules given above is not compatible.
...
A function whose return type is void cannot return a value, i.e., it cannot contain a statement of the form "return expr;"
...
If e is an integer constant, then its type is int.
If e is an identifier, then the type of e is the type of that identifier; if e is an array element, then the type of e is the type of the elements of that array.
If e is a function call, then the type of e is the return type for that function.
If e is an expression of the form e1 + e2, e1 - e2, e1 * e2, e1 / e2, or -e1, then the type of e is int.
If e is an expression of the form e1 >= e2, e1 <= e2, e1 > e2, e1 < e2, e1 == e2, or e1 != e2 then the type of e is bool.
if e is an expression of the form e1 && e2, e1 || e2, or !e1, then the type of e is bool.
A string constant has the type "array of char".
The rules for type checking expressions are given by the following:
The type of the index in an array reference must be compatible with int.
Each actual parameter of a function call must be compatible with the corresponding formal parameter.
The subexpressions associated with the operators +, -, *, /, <=, >=, <, >, ==, and != must be compatible with int.
The subexpressions associated with the operators &&, ||, and ! must be of type bool.
3.3.3. Statements
Only variables of type char or int, or elements of arrays, can be assigned to; the type of the right hand side of an assignment must be compatible with the type of the left hand side of that assignment.
The type of the expression in a return statement in a function must be compatible with the return type of that function.
The type of the conditional in if, for, and while statements must have type bool.
Each actual parameter of a function call must be compatible with the corresponding formal parameter.
...
String Constants
A string constant "a_1 ... a_n" is an array of characters containing n+1 elements, whose first n elements are the corresponding characters in the string, and whose last element is the NUL character \0.
Arrays
An arrays of size n consists of n elements, each occupying an amount of storage equal to that required for the type of the array element, laid out contiguously in memory.
4.2. Expressions
